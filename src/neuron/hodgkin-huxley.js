...
function validateStep(dt) {
	finiteNumber(dt, 'dt');
	if (dt <= 0 || dt > 0.1) {
		throw new RangeError('dt must be in (0, 0.1] ms');
	}
	return dt;
}
...
class HodgkinHuxleyCompartment {
	constructor(options = {}) {
		...
		if (this.parameters.capacitance <= 0) {
			throw new RangeError('capacitance must be positive');
		}
		for (const conductance of ['gNa', 'gK', 'gLeak']) {
			if (this.parameters[conductance] < 0) {
				throw new RangeError(`${conductance} must be non-negative`);
			}
		}
		...
		for (const gate of ['m', 'h', 'n']) {
			if (this[gate] < 0 || this[gate] > 1) {
				throw new RangeError(`${gate} must be in [0, 1]`);
			}
		}
	}
	...
	step(dt, inputCurrent = 0) {
		validateStep(dt);
		const derivative = this.derivatives(inputCurrent);
		this.voltage += dt * derivative.voltage;
		this.m = Math.min(1, Math.max(0, this.m + dt * derivative.m));
		this.h = Math.min(1, Math.max(0, this.h + dt * derivative.h));
		this.n = Math.min(1, Math.max(0, this.n + dt * derivative.n));
...
function simulate({durationMs = 50, dt = 0.01, current = () => 0, compartment} = {}) {
	...
	if (typeof current !== 'function') {
		throw new TypeError('current must be a function');
	}

	const neuron = compartment || new HodgkinHuxleyCompartment();
	if (!(neuron instanceof HodgkinHuxleyCompartment)) {
		throw new TypeError('compartment must be a HodgkinHuxleyCompartment');
	}

	const steps = Math.ceil(durationMs / dt);
	if (steps > MAX_STEPS) {
		throw new RangeError(`simulation exceeds ${MAX_STEPS} steps`);
	}
	...
	for (let index = 0; index <= steps; index += 1) {
		...
		if (index < steps) {
			neuron.step(Math.min(dt, durationMs - timeMs), current(timeMs));
		}
	}
	return samples;
}
